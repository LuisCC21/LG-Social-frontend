...
export const PostComentarios = ({ post }) => {
  if (!post._id)
    return (
      <p className='text-center font-bold mb-10'>Publicacion No Encontrada!</p>
    )

  const [yaLike, setYaLike] = useState(false)
  const [yaDisLike, setYaDisLike] = useState(false)
  const { descripcion, createdAt, updatedAt, _id, like, disLike, creador } =
    post
  const { nombre, genero, _id: idCreador } = post.creador
  const {
    aumentarLikes,
    aumentarDisLikes,
    restarLikes,
